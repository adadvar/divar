...
import { SiderbarBtnElementDragOverlay } from "./siderbarBtnElement";
...
const DragOverlayWrapper = () => {
    ...
    const isSidebarBtnElement =
        draggedItem?.data?.current?.isDesingerBtnElement;

    if (isSidebarBtnElement) {
        const type = draggedItem.data?.current?.type as ElementsType;
        node = (
            <SiderbarBtnElementDragOverlay formElement={FormElements[type]} />
        );
    }
    ...
    if (isDesignerElement) {
        ...
        const element = designerElements.find((el) => el.id == elementId);
